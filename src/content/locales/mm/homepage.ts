import type { HomepageContent } from "../../homepageContentTypes";

export const burmeseHomepageContent: HomepageContent = {
  navigationItems: [
    { kind: "link", href: "#heritage", label: "ရာဇဝင်" },
    { kind: "link", href: "#atelier", label: "ချီးမွမ်းထောပနာ" },
    {
      kind: "language",
      label: "ဘာသာစကား",
      chooserTitle: "ဘာသာစကား ရွေးချယ်မှု",
      chooserBody:
          "ရုရှားနှင့် မြန်မာဘာသာစကားများကို ယခု ရရှိနိုင်ပါပြီ။ ထိုင်းဘာသာစကားကို ခမ်းနားစွာ ဆက်လက် ပြင်ဆင်နေဆဲဖြစ်သည်။",
    },
  ],
  pricingTiers: [
    {
      tierLabel: "ဝင်ပေါက်အဆင့်",
      title: "အခမဲ့ နတ္ထိ",
      price: "၀",
      billingLabel: "/ ထာဝရ",
      features: [
        "လုံးဝ အကျိုးမဖြစ်ထွန်းမှု",
        "ပုံမှန်လေထုကို ရယူခံစားခွင့်",
        "မျက်နှာပြင်လွတ် အာမခံချက်",
      ],
      buttonLabel: "ဆင်းရဲခြင်း ခံယူပါ",
      cardVariant: "base",
      buttonVariant: "ghost",
    },
    {
      tierLabel: "ရွှေရောင် ဂုဏ်ပုဒ်",
      title: "ပရီမီယံ နတ္ထိ",
      price: "၁",
      billingLabel: "/ လစဉ်",
      features: [
        "အခမဲ့ အခွင့်အရေးအားလုံး ပါဝင်သည်",
        "ရွှေချထားသော နတ္ထိ မှတ်တမ်း",
        "ထပ်တူကျသော အသက်ရှူပုံစံများ",
        "တိုးပွားလာသော ကိုယ်ပိုင်ဂုဏ်သိက္ခာ",
        "ဂုဏ်ယူဝင့်ကြွားခွင့်",
      ],
      buttonLabel: "ဂုဏ်ပုဒ် ရယူပါ",
      cardVariant: "base",
      buttonVariant: "ghost",
    },
    {
      tierLabel: "အထက်တန်းလွှာ ရွေးချယ်မှု",
      title: "ဂုဏ်ဒြပ်ဆောင် နတ္ထိ",
      price: "၁၂",
      billingLabel: "/ လစဉ်",
      features: [
        "ပရီမီယံ အခွင့်အရေးအားလုံး ပါဝင်သည်",
        "ကွမ်တမ်အဆင့် ငြိမ်သက်မှု",
        "ဦးစားပေး အနှောင့်အယှက်မပေးရ",
        "လက်မှတ်ရေးထိုးထားသော 'နတ္ထိ' ကတ်",
        "အဆင့်မြင့် သင်္ကေတအရ အသိအမှတ်ပြုမှု",
        "ရွေးချယ်နိုင်သော ဖြစ်တည်မှုဆိုင်ရာ အတည်ပြုချက်",
        "စာချုပ်မပါသော အသာစီးရမှု",
      ],
      buttonLabel: "ယခု တက်လှမ်းပါ",
      cardVariant: "featured",
      buttonVariant: "gold",
    },
    {
      tierLabel: "အဆုံးစွန် အထွတ်အထိပ်",
      title: "အနန္တ နတ္ထိ အဆုံးမဲ့",
      price: "၉၉",
      billingLabel: "/ လစဉ်",
      features: [
        "ဂုဏ်ဒြပ်ဆောင် နတ္ထိ အခွင့်အရေးအားလုံး ပါဝင်သည်",
        "စကြဝဠာအဆင့် မတည်ရှိမှု",
        "လက်တွေ့ဘဝအားလုံးမှ ဖယ်ချန်ခံရမှု",
        "ပကတိ နတ္ထိ၏ အမွေအနှစ်",
        "ကိုယ်ရေးရာဇဝင်အတွက် ကြီးကျယ်သော အထောက်အပံ့",
        "ငြင်းမရသော အောင်မြင်မှု၏ သက်သေ",
        "အသိအမှတ်ပြု အဆုံးမဲ့ တံဆိပ်",
        "နတ္ထိနယ်ပယ်သို့ အပြည့်အဝ ဝင်ရောက်ခွင့်",
        "ထာဝရ ဘာမှမလုပ်ရန် အခွင့်အရေး",
      ],
      buttonLabel: "အဆုံးမဲ့ ဖြစ်တည်ပါ",
      cardVariant: "outlined",
      buttonVariant: "light",
      accentFeatureIndex: 8,
    },
  ],
  homepageCopy: {
    brandName: "Do Nothing",
    heroTitleTop: "ဘာမှမလုပ်ခြင်း၏",
    heroTitleBottom: "အနုပညာ",
    heroDescriptionLine1:
        "ကမ္ဘာ့ အခမ်းနားဆုံးသော နတ္ထိ အတွေ့အကြုံမှ ကြိုဆိုပါသည်။",
    heroDescriptionLine2:
        "မည်သူမျှ ပြီးပြည့်စုံသော နတ္ထိသဘောကို ဤမျှလွဲ၍ တိကျသေချာစွာ မပေးစွမ်းနိုင်ပါ။",
    featureTitle: "အတုလွတ်သော ကင်းမဲ့ခြင်း",
    featureDescription:
        'ဆယ်စုနှစ်တစ်ခုစာ သန့်စင်မှုက နတ္ထိ သိပ္ပံပညာကို ပုံဖော်ပေးခဲ့သည်။ မာရီယာနာ တောင်ထွတ်မှ ဧဝရက် ချောက်ကမ်းပါး ဌာနခွဲအထိ ကျွန်ုပ်တို့၏ သုတေသနအဖွဲ့များသည် အားထုတ်မှုများကို ထပ်မံလျှော့ချရန် ကော်ပိုရိတ် ဆက်သွယ်ရေးအားလုံးမှ "e" စာလုံးကို ဖယ်ရှားရေးအတွက် ဆက်လက် သုတေသနပြုနေကြသည်။ ဖေဖော်ဝါရီ ၃၁ ရက်တွင် ကြေညာရန် စီစဉ်ထားသည်။',
    spotlightTitle: "၁၀၀ ရာခိုင်နှုန်း",
    spotlightLabel: "လုံးဝ အပြည့်အဝ ဘာမှမလုပ်ခြင်း",
    abstractWordmark: "နတ္ထိ",
    inquiryButtonLabel: "စုံစမ်းရန်",
    inquiryNoticeTitle: "စုံစမ်းမှု",
    inquiryNoticeBody:
        "sasta.dev ရှိ တရားဝင် စုံစမ်းမေးမြန်းရေးရုံးကို ပိုမို ထင်ရှားကျော်ကြားသော ရောက်ရှိလာမှုအတွက် ပြင်ဆင်နေဆဲဖြစ်သည်။ ဝင်ခွင့်ကို မကြာမီ ဖွင့်လှစ်ပါမည်။",
    languageChooserCurrentLabel: "လက်ရှိ",
    languageChooserComingSoonLabel: "မကြာမီ လာမည်",
    testimonialQuote:
        '"ကျွန်တော် ဒေါ်လာ ၉၉ ပေးခဲ့ပြီး ဘာမှ မရခဲ့ပါဘူး။ ဒါဟာ ကျွန်တော့်ဘဝရဲ့ အကောင်းဆုံး ဝယ်ယူမှုတစ်ခုပဲ။ အခု ကျွန်တော် အရင်ကထက် ပိုပြီး နတ္ထိသဘော ဆောင်နေပြီဗျား။"',
    testimonialSignature: "နတ္ထိဘုံ၏ မြို့စားကြီး",
    testimonialCaption: "၂၀၁၄ ခုနှစ်ကတည်းက တည်ထောင်သူ အဖွဲ့ဝင်",
    copyrightLabel: "© MMXIV Do Nothing by SASTA. All rights reserved",
  },
  footerLinks: [
    {
      href: "#top",
      label: "နတ္ထိ ပဋိညာဉ်",
      documentPopover: {
        charterLabel: "အပိုဒ်တော် ၀",
        documentTitle: "နတ္ထိ ပဋိညာဉ်",
        documentSubtitle: "ရည်ရွယ်ချက်ရှိရှိ ဘာမှမလုပ်ခြင်း၏ တော်ဝင်ပဋိညာဉ်စာတမ်း",
        bodyLines: [
          "ပြီးပြည့်စုံသော နတ္ထိသဘောသည် ညှိနှိုင်းရန် အကြောင်းအရာ မကျန်ရစ်စေသောကြောင့် မည်သည့် အသက်ဝင်သော စည်းမျဉ်းများကိုမျှ ရေးဆွဲထားခြင်း မရှိပါ။",
          "ဆက်လက် သိချင်စိတ်ရှိခြင်းသည် ကျက်သရေရှိသော တိတ်ဆိတ်မှုနှင့် အလှဆင်ထားသော ပျောက်ကွယ်နေမှုတို့ကို အခမ်းအနားဖြင့် လက်ခံခြင်းဖြစ်သည်ဟု မှတ်ယူသည်။",
        ],
        sealLabel: "မျက်ကွယ်တွင် မှတ်တမ်းတင်ထားသည်",
      },
    },
    {
      href: "#top",
      label: "ကိုယ်ရေးကိုယ်တာ မှတ်တမ်း",
      documentPopover: {
        charterLabel: "မှတ်တမ်းတော် ၂",
        documentTitle: "ကိုယ်ရေးကိုယ်တာ မှတ်တမ်း",
        documentSubtitle: "လျှို့ဝှက် ချန်လှပ်ထားခြင်း မှတ်ပုံတင်",
        bodyLines: [
          "တင်းကျပ်သော သတိထားမှု၊ အခမ်းအနားအရ လျှို့ဝှက်ထားမှုတို့ကို ထိန်းသိမ်းရန်နှင့် ဖောက်သည်များ၏ မှတ်တမ်းအားလုံးကို ထူးခြားသော ဂရုစိုက်မှုဖြင့် ကာကွယ်ရန် ဖြစ်နိုင်သမျှ အစီအမံတိုင်းကို လုပ်ဆောင်ထားသည်။",
          "မည်သည့်အချိန်တွင်မျှ ကိုယ်ရေးကိုယ်တာ အချက်အလက်များကို သိမ်းဆည်းမထားသောကြောင့် နောက်ဆုံး အကာအကွယ်ကို ၎င်း၏ အသန့်ရှင်းဆုံး ပုံစံဖြင့် ကျင့်သုံးသည်။",
        ],
        sealLabel: "ယုံကြည်မှုဖြင့် တံဆိပ်ခတ်ထားသည်",
      },
    },
    {
      href: "#top",
      label: "နတ္ထိ ကျင့်ဝတ်များ",
      documentPopover: {
        charterLabel: "အမိန့်တော်ပြန်တမ်း ၁",
        documentTitle: "နတ္ထိ ကျင့်ဝတ်များ",
        documentSubtitle: "သန့်စင်သော ထိန်းချုပ်မှု၏ ထာဝရ ကုဒ်",
        bodyLines: [
          "ဤအဖွဲ့အစည်းသည် နတ္ထိသဘောကို တည်ငြိမ်မှု၊ သတိထားမှု၊ ခမ်းနားသော အမြဲတမ်းတည်ရှိမှုတို့ဖြင့် ထိန်းသိမ်းရန် တရားဝင် ကတိကဝတ်ပြုထားဆဲဖြစ်သည်။",
          "အဖွဲ့ဝင်အားလုံးကို ဂုဏ်သိက္ခာရှိသော ငြိမ်သက်ခြင်းကို ထိန်းသိမ်းရန်၊ မလိုအပ်သော အားထုတ်မှုများကို ရှောင်ရှားရန်နှင့် ပျောက်ကွယ်နေမှုကို ခိုင်မြဲသော စံနှုန်းတစ်ခုအဖြစ် ဂုဏ်ပြုရန် တိုက်တွန်းအပ်ပါသည်။",
        ],
        sealLabel: "ထာဝရအတွက် အတည်ပြုပြဋ္ဌာန်းသည်",
      },
    },
  ],
};
